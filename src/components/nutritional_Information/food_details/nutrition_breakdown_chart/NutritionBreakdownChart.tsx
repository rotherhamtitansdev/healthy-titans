...
const NutritionBreakdownChart = (props: { name: string }) => {
  ...
  useEffect(() => {
    ...
    const ChartData = NutritionalBreakdownChartData.find((object) => {
      if (object.name === name) { return object.data; }
    });
  ...
  return (
    <div className="basis-1/2">
      ...
      <div className="bg-white rounded-3xl md:w-[36rem] md:h-[58rem]">
        {getLoading ? <ClipLoader color="red" loading={getLoading} size={150} />
          : (
            <div>
              <div className="mx-12 my-8 font-quicksand">
                <p className="text-[24px] text-nutritionInformationHeader font-semibold">Nutrition Information</p>
              ...
              <div className="mx-12 my-8 font-quicksand">
                ...
                <p className="text-[36px] text-titansDarkBlue font-semibold">{getCalories}</p>
                <p className="text-[20px] text-titansDarkBlue">Kcal</p>
              </div>
              <VictoryChart
                height={chartSizing.chartHeight}
              >
                <VictoryBar
                  data={[{
                    x: 1,
                ...
                <VictoryAxis
                  ...
                  tickLabelComponent={<VictoryLabel style={{ fontFamily: "Quicksand", fontSize: "14px" }} />}
                />
              </VictoryChart>
            </div>
          )}
