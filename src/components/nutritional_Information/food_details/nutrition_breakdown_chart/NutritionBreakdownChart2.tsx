...
const NutritionBreakdownChart = (props: { name: string }) => {
  ...
  const [getChartAmountLabels, setChartAmountLabels] = useState<number[]>([]);
  ...
  useEffect(() => {
    FirebaseAPI.fetchNutritionChartData(name).then((data) => {
      if (data !== undefined) {
        ...
        setChartAmountLabels([data.Protein, data.Fat, data.Saturates, data.Sugars, data.Salt]);
      }
    });
  }, []);
  ...
  useEffect(() => {
    if (!isFirstRender) {
      setChartColours(ProcessNutritionData(getChartAmountLabels));
      setLoading(false);
    }
  }, [getNutritionData]);
